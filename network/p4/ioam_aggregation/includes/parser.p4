...
parser MyParser(packet_in packet,
    ...
    state parse_ioam_pto {
        ...
        transition parse_ipv6_option_aggregation;
    }

    state parse_ipv6_option_aggregation {
        packet.extract(hdr.ipv6_option_aggregation);
        transition select(hdr.ipv6_option_aggregation.optionType) {
            HOP_BY_HOP_IOAM_OPTION: parse_ioam_option_aggregation;
        }
    }

    state parse_ioam_option_aggregation {
        packet.extract(hdr.ioam_option_aggregation);
        transition select(hdr.ioam_option_aggregation.ioamOptType) {
            IOAM_AGGREGATION_OPTION_TYPE: parse_ioam_aggregation;
        }
    }
