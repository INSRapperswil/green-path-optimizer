control process_ioam_init(inout headers hdr,
    ...
    action ioam_init_metadata() {
        ...
        meta.ioamTemplateMeta.templateID = IOAM_AGGREGATION_TEMPLATE;

        // In case of multiple templates this can be set dynamically 
        // depending on bits in the IP header (e.g. last n bits in the checksum)
        meta.ioamTemplateMeta.templateLength = IOAM_AGGREGATION_TEMPLATE_LENGTH;

        // In case of multiple templates this can be set dynamically 
        // depending on bits in the IP header (e.g. last n bits in the checksum)
        meta.ioamTemplateMeta.paddingLength = AGGREGATION_PADDING;

        // IOAM Option header (2 Bytes) + Template Option Header (4 Bytes) + Length of Templates (times 4 because the fields stores the number of 4-octet units) 
        meta.ioamTemplateMeta.optionLength = 2 + 4 + 4 * meta.ioamTemplateMeta.templateLength;
    }
